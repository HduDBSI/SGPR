...
class RetrieverDataset:
    def __init__(
        ...
        # 【核心优化 1】：将 emb_dict 保存为类的属性，而不是直接塞进每个样本里
        self.emb_dict = self._load_emb(
            dataset_name, config['dataset']['text_encoder_name'], split)

        # Put everything together.
        self._assembly(
            processed_dict_list, triple_score_dict, skip_no_path)
    ...
    def _load_emb(self, dataset_name, text_encoder_name, split):
        ...
        dict_file = torch.load(file_path, map_location='cpu')
        return dict_file
    ...
    def _assembly(
        ...
        for i in tqdm(range(len(processed_dict_list))):
            ...
            # 【核心优化 3】：移除这行导致内存爆炸的代码！
            # 以前的做法：sample_i.update(emb_dict[sample_i_id]) 
    ...
    def __getitem__(self, i):
        sample = self.processed_dict_list[i].copy()
        sample_id = sample['id']
        sample.update(self.emb_dict[sample_id])
        return sample
