...
# def train_epoch(device, train_loader, model, optimizer,margin):
#     model.train()
#     epoch_loss = 0
...
#     return log_dict
def train_epoch(device, train_loader, model, optimizer, margin):  # NEW: 添加一个margin超参数
    model.train()
    epoch_loss = 0

    margin_loss_fn = torch.nn.MarginRankingLoss(margin=margin, reduction='mean')
    ...
    for sample in tqdm(train_loader):
        ...
        pred_triple_logits = model(
            h_id_tensor, r_id_tensor, t_id_tensor, q_emb, entity_embs,
            num_non_text_entities, relation_embs, topic_entity_one_hot
        ).squeeze(-1)  # squeeze to shape [num_triples]

        positive_mask = (target_triple_probs > 0).to(device)
        negative_mask = ~positive_mask

    
        positive_scores = pred_triple_logits[positive_mask]
        negative_scores_all = pred_triple_logits[negative_mask]

        num_positives = len(positive_scores)
        num_negatives = len(negative_scores_all)

        if num_positives == 0 or num_negatives == 0:
            continue

        positive_scores_expanded = positive_scores.repeat_interleave(num_negatives)
        negative_scores_expanded = negative_scores_all.repeat(num_positives)

        target = torch.ones_like(positive_scores_expanded)

        loss = margin_loss_fn(positive_scores_expanded, negative_scores_expanded, target)
...
#         train_log_dict = train_epoch(device, train_loader, model, optimizer, margin=margin)
...
def main(args):
    ...
    # 动态将终端传入的消融参数注入到 config 的 retriever 配置中
    config['retriever']['ablation_no_gate'] = (args.ablation == 'no_gate')
    config['retriever']['ablation_sum_aggr'] = (args.ablation == 'sum_aggr')
    config['retriever']['ablation_no_structure'] = (args.ablation == 'no_structure')
    ...
    # 在 wandb 命名上加入消融标识，方便你在面板中区分实验
    exp_prefix = config['train']['save_prefix']
    ablation_str = f"_{args.ablation}" if args.ablation != 'none' else ""
    exp_name = f'{exp_prefix}{ablation_str}_{ts}'
    ...
    # 这里会自动把注入进去的消融参数通过 **kwargs 传给 Retriever
    model = Retriever(emb_size, **config['retriever']).to(device)
    optimizer = Adam(model.parameters(), **config['optimizer'])
    margin = config['train'].get('margin', 1.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ablation', type=str, default='none',
                        choices=['none', 'no_gate', 'sum_aggr', 'no_structure'], 
