...
Ablation Studies
To run ablation experiments, use the --ablation argument during training:


# Main method (default: sum aggregation)
```bash
python train.py -d webqsp
```
# Ablation: w/o Relation Gating
```bash
python train.py -d webqsp --ablation no_gate
```
# Ablation: Mean Aggregation
```bash
python train.py -d webqsp --ablation mean_aggr
```
# Ablation: w/o Structural Features
```bash
python train.py -d webqsp --ablation no_structure
```